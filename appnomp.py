                ...
                if hasattr(r, 'boxes') and r.boxes is not None:
                    for box in r.boxes:
                        ...
                        if class_name.lower() == 'person':
                            ...
                            logger.info(f"✅ Person detected via fallback (conf: {confidence:.2f})")
                            logger.info(f"   📦 Person bbox: ({x1},{y1}) to ({x2},{y2})")
                            
                            # Estimate hand regions (both sides of person)
                            person_width = x2 - x1
                            person_height = y2 - y1
                            
                            # Assume hands are in lower-middle area of person detection
                            # Create two hand regions (left and right)
                            hand_regions = []
                            
                            # Left hand region (person's right side in image)
                            left_hand_x = x1 + int(person_width * 0.1)
                            left_hand_y = y1 + int(person_height * 0.4)
                            left_hand_size = int(min(person_width, person_height) * 0.4)
                            
                            hand_regions.append({
                                "x_min": max(0, left_hand_x),
                                "y_min": max(0, left_hand_y),
                                "x_max": min(w, left_hand_x + left_hand_size),
                                "y_max": min(h, left_hand_y + left_hand_size),
                                "side": "left"
                            })
                            
                            # Right hand region (person's left side in image)
                            right_hand_x = x2 - int(person_width * 0.5)
                            right_hand_y = y1 + int(person_height * 0.4)
                            right_hand_size = int(min(person_width, person_height) * 0.4)
                            
                            hand_regions.append({
                                "x_min": max(0, right_hand_x),
                                "y_min": max(0, right_hand_y),
                                "x_max": min(w, right_hand_x + right_hand_size),
                                "y_max": min(h, right_hand_y + right_hand_size),
                                "side": "right"
                            })
                            
                            # Use the hand region closest to center of image
                            center_x, center_y = w // 2, h // 2
                            best_hand = min(hand_regions, key=lambda r: 
                                abs((r["x_min"] + r["x_max"]) // 2 - center_x) +
                                abs((r["y_min"] + r["y_max"]) // 2 - center_y)
                            )
                            
                            wrist_x = (best_hand["x_min"] + best_hand["x_max"]) // 2
                            wrist_y = best_hand["y_min"] + (best_hand["y_max"] - best_hand["y_min"]) // 3
                            
                            logger.info(f"   📍 Estimated wrist position: ({wrist_x}, {wrist_y})")
                            logger.info(f"   📦 Hand bbox: ({best_hand['x_min']},{best_hand['y_min']}) to ({best_hand['x_max']},{best_hand['y_max']})")
                            ...
                            return {
                                ...
                                "keypoints_count": 0,
                                "message": "Person detected via fallback, hand region estimated",
                                "method": "fallback"
                            }
